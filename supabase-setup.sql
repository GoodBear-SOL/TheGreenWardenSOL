/* ============================================================
   THE GREEN WARDEN — SUPABASE MASTER SETUP / REPAIR SCRIPT (v4)
   Safe to re-run.

   REFERRAL SYSTEM — FINAL:
   1. Referral accepted
      -> REFERRER gets +200 WP immediately.

   2. Referred user reaches a 7-day streak
      -> REFERRER gets an additional +300 WP.

   3. Maximum reward per referral
      -> +500 WP total.

   4. Each referral reward type can only be awarded once.

   5. Reward record + profile WP update happen inside the
      same database transaction.

   6. Existing referral rows are NOT automatically credited.
      This prevents accidental historical payouts.

   Clients have NO direct write access to tables.
   All writes are performed through SECURITY DEFINER RPCs.
   ============================================================ */
...
/* ============================================================
   CLEAN DUPLICATES BEFORE UNIQUE INDEXES
   ============================================================ */
...
DELETE FROM public.task_claims a
USING public.task_claims b
WHERE a.user_id = b.user_id
...
CREATE UNIQUE INDEX IF NOT EXISTS task_claims_user_task_unique
...
/* ============================================================
   REFERRAL REWARD SAFETY CONSTRAINTS
   ============================================================ */
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'referral_rewards_valid_type_amount'
      AND conrelid = 'public.referral_rewards'::regclass
  ) THEN

    ALTER TABLE public.referral_rewards
      ADD CONSTRAINT referral_rewards_valid_type_amount
      CHECK (
        (reward_type = 'initial_referral' AND wp_awarded = 200)
        OR
        (reward_type = 'active_referral' AND wp_awarded = 300)
      )
      NOT VALID;
...
BEGIN
  ...
  FOREACH f IN ARRAY ARRAY[
    ...
    'public.claim_checkin()',
    'public.record_portal_activity()',
    'public.get_task_board()',
    'public.submit_task_for_review(text,text)',
    'public.get_admin_task_reviews()',
    'public.approve_task_claim(uuid,text)',
    'public.reject_task_claim(uuid,text)',
    'public.get_leaderboard(integer)',
    'public.get_admin_leaderboard()',
    'public.get_admin_profile(uuid)',
    'public.update_profile(text,text)',
    'public.update_profile(text,text,text)',
    'public.generate_referral_code()',
    'public.get_my_referral_code()',
    'public.get_my_referral_data()',
    'public.attach_referral(text)',
    'public.qualify_referral(uuid)',
    'public.qualify_my_referral()',
    'public.get_my_referral_list()',
    'public.get_my_referral_activity()',
...
AS $$
  SELECT GREATEST(
    ...
    EXTRACT(
      EPOCH FROM (
        ((public.manila_today() + 1)::timestamp AT TIME ZONE 'Asia/Manila')
        - now()
      )
    )::integer
  );
...
CREATE FUNCTION public.effective_streak(
  p_last date,
  p_streak integer
)
RETURNS integer
...
AS $$
  SELECT CASE
    ...
    WHEN p_last >= public.manila_today() - 1
      THEN COALESCE(p_streak,0)
    ELSE 0
  END;
$$;
...
IMMUTABLE
AS $$
  SELECT btrim(COALESCE(p_wallet,'')) ~
    '^[1-9A-HJ-NP-Za-km-z]{32,44}$';
$$;


/* ============================================================
   REFERRAL QUALIFICATION — INTERNAL
   ============================================================ */

/*
   IMPORTANT:

   This function does NOT perform an auth check.

   It is only called internally by trusted SECURITY DEFINER
   functions after the authenticated user has already been
   validated.

   +300 is awarded only when the referred user has an effective
   streak of at least 7.

   The referral row is locked FOR UPDATE to prevent concurrent
   qualification attempts.

   The reward insert and profile WP update occur in the same
   database transaction.
*/

CREATE FUNCTION public._qualify_referral_internal(
  p_referred uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  v_streak integer;
  v_reward integer := 300;
  v_reward_id bigint;
  v_rows integer;
BEGIN
  IF p_referred IS NULL THEN
    RETURN jsonb_build_object('rewarded', false, 'qualified', false, 'error', 'MISSING_REFERRED');
  END IF;
  ...
  THEN
    ...
    SET
      active_reward = COALESCE(active_reward, v_reward),
      active_rewarded_at = COALESCE(active_rewarded_at, now()),
      qualified_at = COALESCE(qualified_at, now())
    WHERE id = v_ref.id
      AND (active_rewarded_at IS NULL OR qualified_at IS NULL);
...
BEGIN
  ...
  IF p_referred_user IS DISTINCT FROM auth.uid()
  ...
  RETURN public._qualify_referral_internal(
    p_referred_user
  );
...
BEGIN
  ...
  v_streak :=
    CASE
      WHEN v_profile.last_checkin = v_today THEN COALESCE(v_profile.streak, 0)
      WHEN v_profile.last_checkin = v_today - 1 THEN COALESCE(v_profile.streak, 0) + 1
      ELSE 1
    END;
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'status', COALESCE(tc.status,'approved'),
...
CREATE FUNCTION public.approve_task_claim(
  p_user_id uuid,
  p_task_id text
)
RETURNS jsonb
...
CREATE FUNCTION public.reject_task_claim(
  p_user_id uuid,
  p_task_id text
)
RETURNS jsonb
...
CREATE FUNCTION public.get_admin_profile(
  p_user_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_wallet IS NOT NULL
     AND NOT COALESCE(
       v_profile.wallet_bonus_claimed,
       false
     )
  THEN
    v_wallet_bonus := 5;
...
/* ============================================================
   ATTACH REFERRAL — FINAL HARDENED VERSION
   ============================================================ */

/*
   EXACT BEHAVIOR:

   Valid referral accepted:
       Referrer +200 WP.

   No other immediate reward.

   Later, when referred user reaches 7-day streak:
       Referrer +300 WP.

   Total:
       +500 WP maximum per referral.

   IMPORTANT:
   The referral reward insert and the referrer WP update
   happen in the same transaction.

   If anything fails, PostgreSQL rolls back the entire RPC.
*/

CREATE FUNCTION public.attach_referral(
  p_code text
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  INTO v_referrer
  FROM public.referral_codes
  WHERE upper(regexp_replace(referral_code, '[^A-Za-z0-9]', '', 'g')) = v_code
  LIMIT 1;
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users u WHERE u.id = v_referrer) THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'REFERRER_MISSING',
      'message', 'That referral code is no longer valid.'
    );
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM public.referrals WHERE referred_id = v_user) THEN
    BEGIN
      v_qualify := public._qualify_referral_internal(v_user);
    EXCEPTION WHEN OTHERS THEN
      v_qualify := jsonb_build_object('rewarded', false);
    END;
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'REFERRER_PROFILE_NOT_FOUND',
      'message', 'The referrer profile could not be found.'
    );
  END IF;
  ...
  IF v_reward_id IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'REWARD_EXISTS',
      'message', 'This referral was already credited.'
    );
  END IF;

  UPDATE public.referrals
  SET
    initial_reward = v_initial_reward,
  ...
  IF v_rows <> 1 THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'REFERRER_WP_UPDATE_FAILED',
      'message', 'Referral saved but the referrer WP could not be updated.'
    );
  END IF;
  ...
  BEGIN
    v_qualify := public._qualify_referral_internal(v_user);
  EXCEPTION WHEN OTHERS THEN
    v_qualify := jsonb_build_object('rewarded', false, 'error', SQLERRM);
  END;
  ...
  RETURN jsonb_build_object(
    ...
    'wp_awarded', v_initial_reward,
    'total_possible_wp', 500,
    'active_bonus', v_qualify
  );
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'initial_rewarded_at',
          r.initial_rewarded_at,

        'active_reward', r.active_reward,
        'active_rewarded_at',
          r.active_rewarded_at
      )
...
BEGIN
  ...
  FOREACH f IN ARRAY ARRAY[
    ...
    'public.effective_streak(date,integer)',

    'public.wp_for_day(integer)',

    'public.is_current_user_admin()',

    'public.is_valid_solana_address(text)',
  ...
  LOOP
    ...
    /* Internal function is intentionally NOT granted to users. */
...
/* ============================================================
   ...
   Replace your email before running:
...
/* ============================================================
   VERIFICATION — BASIC TABLE COUNTS
   ============================================================ */
...
/* ============================================================
   VERIFICATION — REFERRAL REWARD TYPES
   ============================================================ */

SELECT
  reward_type,
  COUNT(*) AS reward_count,
  SUM(wp_awarded) AS total_wp
FROM public.referral_rewards
...
/* ============================================================
   VERIFICATION — REFERRAL REWARD TOTALS
   ============================================================ */
...
/* ============================================================
   VERIFICATION — FIND ANY REFERRAL OVER 500 WP
...
HAVING SUM(wp_awarded) > 500;


/* ============================================================
   VERIFICATION — FIND INVALID REWARD RECORDS
   Expected result: ZERO ROWS
   ============================================================ */
...
/* ============================================================
   ...
   Immediately:
       ...
       referral_rewards:
         initial_referral = 200

       USER A profile:
         wp +200


   USER B checks in consecutively:
